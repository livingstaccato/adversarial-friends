...
def _pump_output(stream, chunks: list, stop_event: threading.Event) -> None:
...
def _early_failure(argv: list[str], duration: float, reason: str) -> SpawnResult:
    ...
    failed."""
    return SpawnResult(
        argv, None, "", "", duration, False,
        NormalizeResult(None, [reason], False), reason, False,
    )
...
def run_process(argv: list[str], stdin_text: str | None, timeout_s: int,
    ...
    if stdout_thread.is_alive() or stderr_thread.is_alive():
        ...
        stdout_thread.join(timeout=_DRAIN_JOIN_S)
        stderr_thread.join(timeout=_DRAIN_JOIN_S)
    for name, thread in (("stdin", stdin_thread), ("stdout", stdout_thread), ("stderr", stderr_thread)):
        if thread.is_alive():
            ...
            warnings.warn(f"spawn: {name} pump thread for {argv!r} did not exit", RuntimeWarning)
    ...
    return SpawnResult(
        argv, process.returncode, stdout, stderr, duration,
        False, result, failure_reason, orphans_suspected,
    )
